...
def get_doc2vec_vector(fname, seq_list):
    """
    param fname: doc2vec file address.
    ...
    for i in seq_list:
        j = model.docvecs[i]
        doc2vec_feature.append(j)
    return doc2vec_feature


def infer_doc2vec_vector(fname, seq_list, k, s):
    """
    param fname: doc2vec file address.
...
for i in [2**3, 2**4, 2**5, 2**6, 2**7, 2**8, 2**9, 2**10]:
    ...
    # Get embedded features for the training data.
    x_feature = get_doc2vec_vector(fname, seq_list)

    file_name = 'doc2vec_' + str(vector_len) + '.csv'

    save_csv(x_feature, file_name)


    # Get embedded features for non-training data.
    seq_list = extract_seq('') #Prediction sequence file

    x_feature = infer_doc2vec_vector(fname, x_1, k, s)
